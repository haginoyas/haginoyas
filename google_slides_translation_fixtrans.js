...
function getFixedTranslations() {
  try {
    ...
    const customTranslations = fixedTranslationsJson ? JSON.parse(fixedTranslationsJson) : {};
    
    // デフォルト固定翻訳とカスタム固定翻訳をマージ
    return { ...DEFAULT_FIXED_TRANSLATIONS, ...customTranslations };
  } catch (error) {
    console.log('固定翻訳辞書の読み込みエラー:', error);
    return DEFAULT_FIXED_TRANSLATIONS;
  }
}

/**
 * カスタム固定翻訳辞書を保存する（デフォルト固定翻訳は除く）
 * @param {Object} translations カスタム固定翻訳辞書
 */
function saveFixedTranslations(translations) {
  try {
    // デフォルト固定翻訳を除いたカスタム固定翻訳のみを保存
    const customTranslations = {};
    Object.entries(translations).forEach(([key, value]) => {
      if (!DEFAULT_FIXED_TRANSLATIONS[key]) {
        customTranslations[key] = value;
      }
    });
    
    PropertiesService.getScriptProperties().setProperty('FIXED_TRANSLATIONS', JSON.stringify(customTranslations));
  } catch (error) {
...
function showCurrentFixedTranslations() {
  const ui = SlidesApp.getUi();
  const customTranslationsJson = PropertiesService.getScriptProperties().getProperty('FIXED_TRANSLATIONS');
  const customTranslations = customTranslationsJson ? JSON.parse(customTranslationsJson) : {};
  
  let message = '【固定翻訳辞書】\n\n';
  
  // デフォルト固定翻訳を表示
  const defaultEntries = Object.entries(DEFAULT_FIXED_TRANSLATIONS);
  message += `■ デフォルト固定翻訳 (${defaultEntries.length}個):\n`;
  defaultEntries.forEach(([source, target]) => {
    message += `「${source}」→「${target}」\n`;
  });
  
  // カスタム固定翻訳を表示
  const customEntries = Object.entries(customTranslations);
  if (customEntries.length > 0) {
    message += `\n■ カスタム固定翻訳 (${customEntries.length}個):\n`;
    customEntries.forEach(([source, target]) => {
      message += `「${source}」→「${target}」\n`;
    });
    message += '\n※カスタム固定翻訳を削除する場合は、「固定翻訳を削除」メニューを使用してください。';
  } else {
    message += '\n■ カスタム固定翻訳: 未設定';
  }
...
function removeFixedTranslation() {
  ...
  if (customEntries.length === 0) {
    ui.alert('削除可能なカスタム固定翻訳がありません。\n\n※デフォルト固定翻訳（permission、Authentication、authorization）は削除できません。');
    return;
  }
  ...
  const response = ui.prompt(
    '固定翻訳削除',
    '削除したい元の単語を入力してください:\n\n削除可能なカスタム固定翻訳:\n' + 
    customEntries.map(([source, target]) => `「${source}」→「${target}」`).join('\n') +
    '\n\n※デフォルト固定翻訳は削除できません',
    ui.ButtonSet.OK_CANCEL
  );
  ...
  if (response.getSelectedButton() === ui.Button.OK) {
    ...
    // デフォルト固定翻訳の削除を防ぐ
    if (DEFAULT_FIXED_TRANSLATIONS[termToRemove]) {
      ui.alert(`「${termToRemove}」はデフォルト固定翻訳のため削除できません。`);
      return;
    }
    
    if (termToRemove && customTranslations[termToRemove]) {
      delete customTranslations[termToRemove];
      PropertiesService.getScriptProperties().setProperty('FIXED_TRANSLATIONS', JSON.stringify(customTranslations));
      ui.alert(`「${termToRemove}」のカスタム固定翻訳を削除しました。`);
    } else {
      ui.alert('指定された単語がカスタム固定翻訳に見つかりません。');
    }
  }
}
...
function onOpen() {
  SlidesApp.getUi()
    ...
    .addItem('固定翻訳辞書を管理', 'manageFixedTranslations')
    .addItem('固定翻訳を削除', 'removeFixedTranslation')
 ...
 * @param {string} targetLang 翻訳先言語コード（'ja', 'en'など）
 * @returns {string} 固定翻訳を適用したテキスト
 */
function applyFixedTranslations(text, targetLang) {
  ...
  // 各固定翻訳を適用
  Object.entries(fixedTranslations).forEach(([source, target]) => {
    // より正確なマッチングのために単語境界を使用
